...
# 记录每个类中的每个token的计数 {tag:{wordid:num},..}
ClassFeaDic = {}
...
def LoadData():
    ...
    print
    len(ClassFreq), "classes!", len(WordDic), "words!"
...
def LoadModel():
    ...
    infile.close()
    print
    len(ClassProb), "classes!", len(WordDic), "words!"
...
def Predict():
    ...
    while len(sline) > 0:
        ...
        #   wid += 1
        i += 1
        # scoreDic 这篇文章 {classid1:总分数，classid2:总分数2}
...
if len(sys.argv) < 4:
    ...
    # 输出模型
    TrainingDataFile = sys.argv[2]  # nb_data.train
    ModelFile = sys.argv[3]  # model
    LoadData()
    ComputeModel()
    SaveModel()
elif sys.argv[1] == '0':
